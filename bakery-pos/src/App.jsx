...
export default function App() {
  ...
  const deleteOrder  = id => { const o = st.orders.find(x => x.id === id); setConfirm({ title: "\ud83d\uddd1 Delete Order?", msg: `Delete order for ${o.customer}? ($${o.total.toFixed(2)})`, confirmColor: C.red, onConfirm: () => { upd(s => { s.orders = s.orders.filter(x => x.id !== id); const wk = s.currentWeekKey; if (s.weeklyData[wk]) { ITEMS.forEach(it => { s.weeklyData[wk].sold[it.id] = Math.max(0, (s.weeklyData[wk].sold[it.id] || 0) - (o.items[it.id] || 0)); }); if (!o.exempt && (o.paidFull || o.paid >= o.total)) { s.weeklyData[wk].revenue = Math.max(0, (s.weeklyData[wk].revenue || 0) - o.total); } else if (!o.exempt && o.paid > 0) { s.weeklyData[wk].revenue = Math.max(0, (s.weeklyData[wk].revenue || 0) - o.paid); } } ITEMS.forEach(it => { const qty = o.items[it.id] || 0; if (!qty) return; const inMk = s.makeList[it.id] || 0; const restore = Math.min(qty, inMk); if (restore > 0) { s.makeList[it.id] = inMk - restore; if (s.makeList[it.id] <= 0) delete s.makeList[it.id]; s.stock[it.id] = s.makeList[it.id] ? -s.makeList[it.id] : 0; } else { s.stock[it.id] = (s.stock[it.id] || 0) + qty; } }); return s; }); toast2("Order deleted"); } }); };
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "16px 14px" }}>
        ...
        {screen === "report" && <div style={card()}>
          ...
          <div style={{ padding: "16px 18px", ...col({ gap: 16 }) }}>
            ...
            <div style={{ background: C.warm, border: `1.5px solid ${C.soft}`, borderRadius: 10, padding: "13px 16px", ...col({ gap: 8 }) }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontWeight: 700, fontSize: "0.88rem", color: C.brown, textTransform: "uppercase", letterSpacing: 1 }}>💰 Paid Revenue</span><span style={{ fontFamily: "Georgia,serif", fontSize: "1.5rem", fontWeight: 900, color: C.green }}>${rev.toFixed(2)}</span></div>{amtOwed > 0 && <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: `1px dashed ${C.soft}`, paddingTop: 8 }}><span style={{ fontWeight: 700, fontSize: "0.88rem", color: C.red, textTransform: "uppercase", letterSpacing: 1 }}>⏳ Outstanding Balance</span><span style={{ fontFamily: "Georgia,serif", fontSize: "1.5rem", fontWeight: 900, color: C.red }}>${amtOwed.toFixed(2)}</span></div>}</div>
