...
import { loadState, saveState, supabase } from "./supabase";
...
// ── Challah dough logic ──────────────────────────────────────────────────────
// Big challahs = 1 dough each, Small challahs = 0.5 dough each
// Total pool = 16 doughs per week
const CHALLAH_POOL_MAX = 16;
const CHALLAH_BIG_CATS  = ["Big"];    // categories counted as 1 dough
const CHALLAH_SM_CATS   = ["Small"];  // categories counted as 0.5 dough

function doughUsed(items, qtys) {
  let used = 0;
  items.forEach(it => {
    const q = qtys[it.id] || 0;
    if (!q) return;
    const cat = it.cat || it.category || "";
    if (CHALLAH_BIG_CATS.includes(cat)) used += q * 1;
    else if (CHALLAH_SM_CATS.includes(cat)) used += q * 0.5;
  });
  return used;
}

function isChallah(it) {
  const cat = it.cat || it.category || "";
  return CHALLAH_BIG_CATS.includes(cat) || CHALLAH_SM_CATS.includes(cat);
}
...
const inp=(x={})=>({padding:"9px 13px",border:`1.5px solid ${C.soft}`,borderRadius:8,fontSize:"1rem",background:C.cream,color:C.dark,width:"100%",fontFamily:"inherit",boxSizing:"border-box",...x});
...
function AddItemForm({ onAdded }) {
  ...
  const [cat, setCat]       = useState("Big");
  const [price, setPrice]   = useState("");
  const [desc, setDesc]     = useState("");
  const [saving, setSaving] = useState(false);

  const cats = ["Big","Small","Sides","Extras"];

  const save = async () => {
    if (!name.trim() || !price) return;
    setSaving(true);
    const { data, error } = await supabase.from("menu_items").insert([{
      name: name.trim(), category: cat, price: parseFloat(price), description: desc.trim(),
      hidden: false, sort_order: 99,
    }]).select().single();
    setSaving(false);
    if (!error && data) {
      setName(""); setPrice(""); setDesc(""); setCat("Big");
      onAdded(data);
    }
  };

  return (
    <div style={{ background: C.warm, border: `1.5px solid ${C.soft}`, borderRadius:12, padding:"16px 18px", ...col({gap:10}) }}>
      <div style={{ fontFamily:"Georgia,serif", color:C.brown, fontSize:"0.95rem", fontWeight:700 }}>➕ Add New Item</div>
      <div style={g2}>
        <div style={col({gap:4})}>
          <label style={{fontSize:"0.72rem",fontWeight:700,color:C.brown,textTransform:"uppercase",letterSpacing:1}}>Name *</label>
          <input value={name} onChange={e=>setName(e.target.value)} placeholder="Item name" style={inp()} />
        </div>
        <div style={col({gap:4})}>
          <label style={{fontSize:"0.72rem",fontWeight:700,color:C.brown,textTransform:"uppercase",letterSpacing:1}}>Category</label>
          <select value={cat} onChange={e=>setCat(e.target.value)} style={inp()}>
            {cats.map(c=><option key={c}>{c}</option>)}
          </select>
        </div>
        <div style={col({gap:4})}>
          <label style={{fontSize:"0.72rem",fontWeight:700,color:C.brown,textTransform:"uppercase",letterSpacing:1}}>Price *</label>
          <input type="number" min="0" step="0.01" value={price} onChange={e=>setPrice(e.target.value)} placeholder="0.00" style={inp()} />
        </div>
        <div style={col({gap:4})}>
          <label style={{fontSize:"0.72rem",fontWeight:700,color:C.brown,textTransform:"uppercase",letterSpacing:1}}>Description</label>
          <input value={desc} onChange={e=>setDesc(e.target.value)} placeholder="Optional" style={inp()} />
        </div>
      </div>
      <button onClick={save} disabled={saving||!name.trim()||!price} style={btn(C.brown,C.cream,{opacity:saving||!name.trim()||!price?0.5:1})}>
...
export default function App() {
  ...
  const [items, setItems]     = useState([]);   // from menu_items table
  ...
  // ── Load items from menu_items table ──────────────────────────────────────
  const loadItems = useCallback(async () => {
    const { data } = await supabase.from("menu_items").select("*").eq("hidden", false).order("sort_order").order("created_at");
    if (data) {
      // Normalize: map category -> cat for internal use
      const normalized = data.map(it => ({ ...it, cat: it.category }));
      setItems(normalized);
      // Ensure fq has keys for all items
      setFq(prev => {
        const n = {};
        normalized.forEach(it => { n[it.id] = prev[it.id] || 0; });
        return n;
      });
    }
  }, []);

  // ── Load state + items on mount ───────────────────────────────────────────
  useEffect(() => {
  ...
  const scheduleSave = useCallback((newState) => {
    ...
    saveTimer.current = setTimeout(() => {
      saveState(newState).then(()=>setSaveError(false)).catch(()=>setSaveError(true));
    }, 800);
  }, []);
  ...
  const ot    = items.reduce((s,it)=>s+(fq[it.id]||0)*parseFloat(it.price),0);
  ...
  const doughInOrder = doughUsed(items, fq);
  ...
  const deleteOrder = id => {
    ...
    setConfirm({ title:"🗑 Delete Order?", msg:`Delete order for ${o.customer}? ($${o.total.toFixed(2)})`, confirmColor:C.red, onConfirm:()=>{
      upd(s => {
  ...
  const openEdit = id => {
    ...
    setEditOrd(o); setEq({...o.items}); setEp(o.paid.toFixed(2)); setEpf(o.paidFull||o.paid>=o.total); setEex(!!o.exempt);
  };
  const saveEdit = () => {
    if (!editOrd) return;
    let t=0; items.forEach(it=>t+=(eq[it.id]||0)*parseFloat(it.price));
    const p=eex?0:parseFloat(ep)||0; const pf=!eex&&(epf||p>=t);
    upd(s => {
    ...
    setEditOrd(null); toast2("Order updated ✓","success");
  };
  ...
  const setStock = id => {
    const val=parseInt(sInp[id]||""); if(isNaN(val)||val<0){toast2("Enter a valid number","error");return;}
  ...
  const startNewWeek = () => setConfirm({ title:"🗓 Start New Week?", confirmColor:C.charity, confirmLabel:"Start New Week", msg:"Clears paid & picked-up orders. Unpaid orders carry over. Stock & make list stay.", onConfirm:()=>{
    ...
    setVw(getWeekKey()); toast2("New week started! 🗓","success");
  }});
  ...
  const logDon  = () => {
    ...
    setDon(""); toast2(`$${amt.toFixed(2)} donation recorded 🕊`,"charity");
  };

  // ── Challah dough pool display (Stock tab) ────────────────────────────────
  // Dough consumed = sum of stock + makeList for challah items
  function doughConsumedFromStock() {
    let used = 0;
    items.forEach(it => {
      if (!isChallah(it)) return;
      const inStock = Math.max(0, st.stock[it.id] || 0);
      const toMake  = Math.max(0, st.makeList[it.id] || 0);
      const total   = inStock + toMake;
      const cat = it.cat || it.category || "";
      if (CHALLAH_BIG_CATS.includes(cat)) used += total * 1;
      else if (CHALLAH_SM_CATS.includes(cat)) used += total * 0.5;
    });
    return used;
  }
    ...
    <div style={{background:C.cream,minHeight:"100vh",fontFamily:"Inter,system-ui,sans-serif",color:C.dark,paddingBottom:80}}>
      ...
      <div style={{maxWidth:920,margin:"0 auto",padding:"16px 14px"}}>
        ...
        {screen==="order" && <div style={col({gap:18})}>
          <div style={card()}>
            <div style={ph}><span>🛒</span> New Order</div>
            <div style={{padding:"16px 18px",...col({gap:14})}}>
              <div style={col({gap:5})}>
                <label style={{fontWeight:700,fontSize:"0.82rem",color:C.brown,textTransform:"uppercase",letterSpacing:1}}>Customer Name</label>
                <input value={custName} onChange={e=>setCustName(e.target.value)} placeholder="Enter customer name…" style={inp()} />
              </div>

              {/* Challah dough pool indicator */}
              {items.some(isChallah) && (
                <div style={{background:doughInOrder>CHALLAH_POOL_MAX?"#fff0f0":C.warm,border:`1.5px solid ${doughInOrder>CHALLAH_POOL_MAX?C.red:C.soft}`,borderRadius:9,padding:"10px 14px",...row({justifyContent:"space-between"})}}>
                  <span style={{fontWeight:700,fontSize:"0.82rem",color:doughInOrder>CHALLAH_POOL_MAX?C.red:C.brown,textTransform:"uppercase",letterSpacing:1}}>🌾 Dough Pool</span>
                  <span style={{fontFamily:"Georgia,serif",fontSize:"1.1rem",fontWeight:900,color:doughInOrder>CHALLAH_POOL_MAX?C.red:C.brown}}>
                    {doughInOrder} / {CHALLAH_POOL_MAX} used
                  </span>
                </div>
              )}
              ...
              {cats.map(cat => {
                ...
                const icon = cat==="Big"?"🍞":cat==="Small"?"🍞":cat==="Sides"?"🫙":"📦";
                return (
                  <div key={cat}>
          ...
          <div style={{paddingBottom:16}}>
            ...
            <div style={{padding:"14px 16px",...col({gap:10})}}>
              {(orderTab==="active"?activeOrders:orderTab==="unpaid"?unpaidPickedUp:archivedOrders).map(o => {
                ...
                const bl = o.total - o.paid;
                // Build item label using dynamic items list
                const det = items.filter(it=>(o.items[it.id]||0)>0).map(it=>`${o.items[it.id]} × ${it.name}`).join(", ")
                  || Object.entries(o.items||{}).filter(([,v])=>v>0).map(([k,v])=>`${v} × ${k}`).join(", ");
                const dt = new Date(o.ts).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"});
                const bg = isPd||o.exempt?"#eafaf1":"#fff8f0";
                const br = isPd||o.exempt?"#a8d5b5":o.paid>0?C.orange:"#f0d5a8";
                const bc = {unpaid:["#fdebd0","#c0392b"],partial:["#fef0e0",C.orange],paid:["#d5f5e3",C.green],exempt:["#f3eeff",C.charity]}[status]||["#fdebd0","#c0392b"];

                return (
                  <div key={o.id} style={{background:bg,border:`1.5px solid ${br}`,borderRadius:10,padding:"13px 15px",...col({gap:6})}}>
                    <div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontWeight:700,fontSize:"0.95rem",color:C.brown}}>👤 {o.customer}</span><span style={{fontSize:"0.7rem",color:"#aaa"}}>{dt}{o.online&&<span style={{marginLeft:6,background:"#e8f4ff",color:"#1a6fa8",borderRadius:20,padding:"1px 7px",fontSize:"0.65rem",fontWeight:700}}>Online</span>}</span></div>
                    <div style={{fontSize:"0.84rem",color:"#666"}}>{det}</div>
                    {o.note && <div style={{fontSize:"0.78rem",color:"#888",fontStyle:"italic"}}>📝 {o.note}</div>}
        ...
        {screen==="bake" && <div style={col({gap:18})}>
          <div style={card()}>
            ...
            <div style={{padding:"16px 18px",...col({gap:12})}}>
              {mke.length===0
                ? <div style={{textAlign:"center",color:"#bbb",padding:20}}>Nothing to make right now 🎉</div>
                : mke.map(([id,qty])=>{
                    const it=items.find(x=>x.id===id)||{name:id};
                    return (
                      <div key={id} style={{display:"flex",alignItems:"center",gap:12,background:C.cream,border:`1.5px solid ${C.orange}`,borderRadius:10,padding:"12px 14px"}}>
                        <div style={{flex:1}}>
                          <div style={{fontWeight:700,fontSize:"0.85rem",color:C.brown,textTransform:"uppercase",letterSpacing:1}}>{it.name}</div>
        ...
        {/* ══════════════════ STOCK TAB ══════════════════ */}
        {screen==="stock" && <div style={col({gap:16})}>

          {/* Challah Dough Pool */}
          {items.some(isChallah) && (() => {
            const doughUsedTotal = doughConsumedFromStock();
            const doughLeft = CHALLAH_POOL_MAX - doughUsedTotal;
            const pct = Math.min(100, (doughUsedTotal / CHALLAH_POOL_MAX) * 100);
            const barColor = doughLeft < 2 ? C.red : doughLeft < 5 ? C.orange : C.green;
            return (
              <div style={{...card(),overflow:"hidden"}}>
                <div style={ph}><span>🌾</span> Challah Dough Pool</div>
                <div style={{padding:"16px 18px",...col({gap:12})}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                      <div style={{fontFamily:"Georgia,serif",fontSize:"1.6rem",fontWeight:900,color:doughLeft<2?C.red:C.brown}}>{doughLeft.toFixed(1)} doughs remaining</div>
                      <div style={{fontSize:"0.78rem",color:"#888"}}>{doughUsedTotal.toFixed(1)} of {CHALLAH_POOL_MAX} used · Big=1 dough · Small=½ dough</div>
                    </div>
                    <div style={{fontFamily:"Georgia,serif",fontSize:"2rem",fontWeight:900,color:barColor}}>{doughUsedTotal.toFixed(1)}/{CHALLAH_POOL_MAX}</div>
                  </div>
                  <div style={{height:12,background:C.soft,borderRadius:10,overflow:"hidden"}}>
          ...
          <div style={card()}>
            ...
            <div style={{padding:"16px 18px",...col({gap:18})}}>
              {cats.map(cat => {
                const catItems = items.filter(i=>i.cat===cat);
                const icon = cat==="Big"?"🍞":cat==="Small"?"🍞":cat==="Sides"?"🫙":"📦";
                return (
                      ...
                      {catItems.map(it => {
                        ...
                        return (
                          <div key={it.id} style={{background:C.cream,border:`1.5px solid ${C.soft}`,borderRadius:10,padding:"13px 15px",...col({gap:8})}}>
                            ...
                            <div style={g2}>
                              <div style={{background:"white",border:`1.5px solid ${stockColor}20`,borderRadius:9,padding:"10px 12px",...col({gap:4})}}>
                                <div style={{fontSize:"0.7rem",fontWeight:700,color:stockColor,textTransform:"uppercase",letterSpacing:1}}>✅ In Stock</div>
                                <div style={{display:"flex",alignItems:"center",gap:6}}>
                                  <button style={qb} onClick={()=>adjStock(it.id,-1)}>−</button>
                                  <span style={{fontFamily:"Georgia,serif",fontSize:"1.8rem",fontWeight:900,color:stockColor,minWidth:36,textAlign:"center"}}>{qty}</span>
                                  <button style={qb} onClick={()=>adjStock(it.id,1)}>+</button>
                                </div>
                                <div style={{display:"flex",gap:5,marginTop:2}}>
                                  <input type="number" min="0" placeholder="set" value={sInp[it.id]||""} onChange={e=>setSInp(p=>({...p,[it.id]:e.target.value}))} style={{width:55,padding:"4px 7px",border:`1.5px solid ${C.soft}`,borderRadius:6,fontSize:"0.85rem",textAlign:"center",background:"white"}} />
                                  <button onClick={()=>setStock(it.id)} style={btn(C.gold,"white",{padding:"4px 10px",fontSize:"0.75rem"})}>Set</button>
                                </div>
                              </div>
                              <div style={{background:"white",border:`1.5px solid ${toMake>0?C.orange+"40":"#eee"}`,borderRadius:9,padding:"10px 12px",...col({gap:4})}}>
                                <div style={{fontSize:"0.7rem",fontWeight:700,color:toMake>0?C.orange:"#bbb",textTransform:"uppercase",letterSpacing:1}}>🔥 Still To Make</div>
                                <div style={{fontFamily:"Georgia,serif",fontSize:"1.8rem",fontWeight:900,color:toMake>0?C.orange:"#ccc",textAlign:"center"}}>{toMake}</div>
                                {toMake>0&&<div style={{fontSize:"0.68rem",color:"#aaa",textAlign:"center"}}>needed for orders</div>}
                              </div>
                            </div>
                          </div>
                        );
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:4,flexWrap:"wrap",gap:8}}>
                <button onClick={()=>setShowAddItem(p=>!p)} style={btn(C.brown,C.cream,{padding:"8px 16px"})}>
                  {showAddItem?"✕ Cancel":"➕ Add Item"}
                </button>
                <button onClick={()=>setConfirm({title:"🗑 Reset Inventory",msg:"Reset all stock to zero?",confirmColor:C.red,onConfirm:()=>{upd(s=>{items.forEach(it=>s.stock[it.id]=0);return s;});toast2("Inventory reset");}})} style={{padding:"6px 14px",background:"transparent",color:C.red,border:`1.5px solid ${C.red}`,borderRadius:7,fontSize:"0.75rem",fontWeight:700,cursor:"pointer",opacity:.7}}>🗑 Reset All</button>
              </div>

              {showAddItem && <AddItemForm onAdded={newItem => {
                setItems(prev=>[...prev,{...newItem,cat:newItem.category}]);
                setShowAddItem(false);
                toast2(`${newItem.name} added ✓`,"success");
              }} />}
            </div>
          </div>
        </div>}
          ...
          <div style={{padding:"16px 18px",...col({gap:16})}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
              ...
              <div style={row({gap:6})}>
                ...
                <span style={{fontSize:"0.78rem",fontWeight:700,color:C.brown,minWidth:80,textAlign:"center"}}>
                  {Math.round((new Date(vw)-new Date(getWeekKey()))/(7*86400000))===0?"This Week":Math.round((new Date(vw)-new Date(getWeekKey()))/(7*86400000))<0?`${Math.abs(Math.round((new Date(vw)-new Date(getWeekKey()))/(7*86400000)))}w ago`:`${Math.round((new Date(vw)-new Date(getWeekKey()))/(7*86400000))}w ahead`}
                </span>
                <button onClick={()=>{const d=new Date(vw+"T00:00:00");d.setDate(d.getDate()+7);setVw(getWeekKey(d));}} style={{width:30,height:30,borderRadius:"50%",border:`1.5px solid ${C.gold}`,background:"white",color:C.gold,fontWeight:700,cursor:"pointer",fontSize:"1rem"}}>›</button>
              </div>
            </div>
            ...
            {cats.map(cat => {
              ...
              return (
                <div key={cat}>
                  <div style={{fontFamily:"Georgia,serif",fontSize:"0.85rem",color:C.brown,fontWeight:700,borderBottom:`1px solid ${C.soft}`,paddingBottom:5,marginBottom:10,letterSpacing:1}}>{icon} {cat}</div>
                  <div style={g2}>{catItems.map(it=>(
            ...
            {/* Revenue block */}
            <div style={{background:C.warm,border:`1.5px solid ${C.soft}`,borderRadius:10,padding:"13px 16px",...col({gap:8})}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontWeight:700,fontSize:"0.88rem",color:C.brown,textTransform:"uppercase",letterSpacing:1}}>💰 Paid Revenue</span>
                <span style={{fontFamily:"Georgia,serif",fontSize:"1.5rem",fontWeight:900,color:C.green}}>${rev.toFixed(2)}</span>
              </div>
              {amtOwed>0&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",borderTop:`1px dashed ${C.soft}`,paddingTop:8}}>
                <span style={{fontWeight:700,fontSize:"0.88rem",color:C.red,textTransform:"uppercase",letterSpacing:1}}>⏳ Outstanding Balance</span>
                <span style={{fontFamily:"Georgia,serif",fontSize:"1.5rem",fontWeight:900,color:C.red}}>${amtOwed.toFixed(2)}</span>
              </div>}
            </div>
            ...
            {(ct>0||totalBilled>0)&&<div style={{background:"linear-gradient(135deg,#f3eeff 0%,#ede0ff 100%)",border:"2px solid #c4a8e8",borderRadius:12,padding:"16px 18px",...col({gap:12})}}>
              ...
              {cow<=0
                ? <div style={{background:"#e8f8ef",border:"1.5px solid #a8d5b5",borderRadius:10,padding:"13px 15px",textAlign:"center",fontFamily:"Georgia,serif",fontSize:"1rem",color:C.green,letterSpacing:1}}>✓ Charity fully paid for this week</div>
                : <>
                  <div style={g2}>
      ...
      {/* ══ Bottom Nav (no Menu tab) ══ */}
      <nav style={{position:"fixed",bottom:0,left:0,right:0,background:C.brown,borderTop:`3px solid ${C.gold}`,display:"flex",zIndex:200}}>
        {[
          {id:"order",  icon:"🛒", label:"Order",  b:0},
          {id:"orders", icon:"📋", label:"Orders", b:unpaidCount},
          {id:"bake",   icon:"🔥", label:"Bake",   b:mke.length},
          {id:"stock",  icon:"📦", label:"Stock",  b:0},
          {id:"report", icon:"📊", label:"Report", b:0},
        ].map(t=>(
      ...
      {editOrd&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.45)",zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
        <div style={{background:"white",borderRadius:14,padding:"22px 20px",maxWidth:500,width:"100%",boxShadow:"0 12px 40px rgba(0,0,0,.3)",borderTop:`5px solid ${C.gold}`,maxHeight:"90vh",overflowY:"auto"}}>
          ...
          {cats.map(cat=>{
            ...
            return (
              <div key={cat}>
                <div style={{fontFamily:"Georgia,serif",fontSize:"0.9rem",color:C.brown,fontWeight:700,borderBottom:`2px solid ${C.soft}`,paddingBottom:5,margin:"12px 0 8px",letterSpacing:1}}>{icon} {cat}</div>
                <div style={g2}>{catItems.map(it=>(
                  <div key={it.id} style={{background:C.cream,border:`1.5px solid ${C.soft}`,borderRadius:9,padding:"10px 12px"}}>
                    <div style={{fontWeight:700,fontSize:"0.78rem",color:C.brown,textTransform:"uppercase",letterSpacing:1,marginBottom:2}}>{it.name}</div>
                    {it.description&&<div style={{fontSize:"0.68rem",color:"#bbb",marginBottom:4}}>{it.description}</div>}
                    <QC value={eq[it.id]||0} onChange={v=>setEq(p=>({...p,[it.id]:Math.max(0,v)}))} />
                  </div>
                ))}</div>
              </div>
            );
          ...
          {!eex&&<div style={{background:C.warm,borderRadius:9,padding:"12px 14px",margin:"12px 0",border:`1.5px solid ${C.soft}`,...col({gap:9})}}>
            <div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontWeight:700,fontSize:"0.82rem",color:C.brown,textTransform:"uppercase"}}>Total</span><span style={{fontFamily:"Georgia,serif",fontSize:"1.3rem",fontWeight:900,color:C.gold}}>${items.reduce((s,it)=>s+(eq[it.id]||0)*parseFloat(it.price),0).toFixed(2)}</span></div>
            ...
            <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer"}}><input type="checkbox" checked={epf} onChange={e=>{setEpf(e.target.checked);if(e.target.checked)setEp(items.reduce((s,it)=>s+(eq[it.id]||0)*parseFloat(it.price),0).toFixed(2));}} style={{width:18,height:18,accentColor:C.green}} /><span style={{fontWeight:700,fontSize:"0.82rem",color:C.green,textTransform:"uppercase"}}>Paid in Full</span></label>
          </div>}
