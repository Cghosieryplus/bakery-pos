...
export default function App() {
  ...
  const deleteOrder  = id => { const o = st.orders.find(x => x.id === id); setConfirm({ title: "🗑 Delete Order?", msg: `Delete order for ${o.customer}? ($${o.total.toFixed(2)})`, confirmColor: C.red, onConfirm: () => { upd(s => { s.orders = s.orders.filter(x => x.id !== id); const wk = s.currentWeekKey; if (s.weeklyData[wk]) { ITEMS.forEach(it => { s.weeklyData[wk].sold[it.id] = Math.max(0, (s.weeklyData[wk].sold[it.id] || 0) - (o.items[it.id] || 0)); }); if (!o.exempt && (o.paidFull || o.paid >= o.total)) { s.weeklyData[wk].revenue = Math.max(0, (s.weeklyData[wk].revenue || 0) - o.total); } else if (!o.exempt && o.paid > 0) { s.weeklyData[wk].revenue = Math.max(0, (s.weeklyData[wk].revenue || 0) - o.paid); } } return s; }); toast2("Order deleted"); } }); };
